...
if( ! function_exists( 'numinous_relative_time' ) ) :
/**
 * Changes date to relative time.
*/
function numinous_relative_time( $a ) {		
    ...
    if( is_numeric( $d ) && $d > 0 ) {				
		if( $d < 3 ) return esc_html__( "right now", 'numinous' ); //if less then 3 seconds
		if( $d < $minute ) return floor($d) . esc_html__( " seconds ago", 'numinous' ); //if less then minute
		if( $d < $minute * 2 ) return esc_html__( "about 1 minute ago", 'numinous' ); //if less then 2 minutes
		if( $d < $hour ) return floor($d / $minute) . esc_html__( " minutes ago", 'numinous' ); //if less then hour
		if( $d < $hour * 2 ) return esc_html__( "about 1 hour ago", 'numinous' ); //if less then 2 hours
		if( $d < $day ) return floor($d / $hour) . esc_html__( " hours ago", 'numinous' ); //if less then day
		if( $d > $day && $d < $day * 2 ) return esc_html__( "yesterday", 'numinous' ); //if more then day, but less then 2 days
		if( $d < $day * 365 ) return floor($d / $day) . esc_html__( " days ago", 'numinous' ); //if less then year
		return esc_html__( "over a year ago", 'numinous' ); //else return more than a year
	}
}
endif;
