...
function xecuter_getPostViews($postID){
    ...
    if($count=='' || $count=='0'){
        ...
        return "0 ". xecuter_t('view');
    }
    return $count.' '. xecuter_t('views') ;
}
...
function xecuter_posts_custom_column_views($column_name, $id){
	if($column_name === 'xecuter_post_views'){
        echo xecuter_getPostViews(get_the_ID());
    }
} 
            ...
            <div class="rd-singin">
                <ul>
					...
					<?php if ( get_option('users_can_register') ) : ?>
					 / <a href="<?php echo esc_url(home_url('/')); ?>wp-login.php?action=register"><?php echo esc_html__( 'Register', 'xecuter' ); ?></a>
					<?php endif; ?> 
                    </li>
               </ul>
            </div>
 ...
 function xecuter_time_elapsed_string( )
{
    ...
    $a_plural = array( 'year'   => esc_html(xecuter_t('years')),
    ...
    foreach ($a as $secs => $str)
    {
        ...
        {
            $r = round($d);
            return $r . ' ' . ($r > 1 ? $a_plural[$str] : $str) . " ". esc_html(xecuter_t('ago'));
        }
    }
}
...
function xecuter_get_time(){
	...
	$time_format = !empty($smof_data['xecuter_time_format' ]) ? $smof_data['xecuter_time_format' ] :'traditional';
 	if(  $time_format == 'traditional'  ){	
		...
		echo xecuter_time_elapsed_string();
